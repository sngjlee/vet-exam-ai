...
**Goal:** Replace `/dashboard` (currently an embedded quiz session) with a rich stats overview hub, and move the quiz session to `/quiz`.

**Architecture:** The new `/dashboard` is a client component that reads from `useStats` + `useDueCountCtx` + `findWeakestCategory` — the same hooks as `/my-stats`. The existing quiz session JSX moves verbatim from `app/dashboard/page.tsx` to `app/quiz/page.tsx`. One new CSS class (`.dashboard-2col`) handles the responsive 2-column layout in the subject bars / CTA row.

**Tech Stack:** Next.js 14 App Router, React 18 hooks, Supabase JS client, CSS custom properties, Tailwind v4 (for utilities like `grid grid-cols-2 sm:grid-cols-4`)
...
|---|---|---|
| `app/quiz/page.tsx` | **Create** | Quiz session (moved from dashboard) |
| `app/dashboard/page.tsx` | **Replace** | New stats overview hub |
| `app/globals.css` | **Modify** | Add `.dashboard-2col` responsive class |

---

## Task 1: Move quiz session to `/quiz`

**Files:**
- Create: `app/quiz/page.tsx`
- Modify (delete content): `app/dashboard/page.tsx` (will be fully replaced in Task 3)

### Steps

- [ ] **Step 1: Copy current dashboard to `/quiz`**

Copy `app/dashboard/page.tsx` to `app/quiz/page.tsx` verbatim. No changes needed — the quiz session logic, imports, and component are identical. Only the export name changes from `Home` to `QuizPage`.

```tsx
// app/quiz/page.tsx
// [exact content of current app/dashboard/page.tsx]
// Change only: export default function QuizPage() {
```

- [ ] **Step 2: Update the NavBar "랜덤 세션" link target**

Verify `components/NavBar.tsx` — it currently links logo to `/dashboard`. No change needed for the logo (logo → stats hub is correct). Add `/quiz` as the target for the random session CTA in the new dashboard (handled in Task 3, not in NavBar).

- [ ] **Step 3: Verify the quiz route loads**

Run `npm run dev`, navigate to `http://localhost:3000/quiz`. Confirm the 5-question quiz session renders and works (category dropdown, start session, answer flow, results screen).

- [ ] **Step 4: Commit**

```bash
git add app/quiz/page.tsx
git commit -m "feat: move quiz session to /quiz route"
```

---

## Task 2: Add `.dashboard-2col` CSS class
...
- [ ] **Step 1: Add the class at the end of the landing page section in globals.css**
...
@media (min-width: 900px) {
  .dashboard-2col { grid-template-columns: 1.2fr 0.8fr; }
}
```
...
git commit -m "feat: add dashboard-2col responsive grid class"
```

---

## Task 3: Build new `/dashboard` stats hub

**Files:**
- Replace: `app/dashboard/page.tsx`

This is the main task. The page is a `"use client"` component with 4 sub-components defined in the same file: `MemoryCurve`, `StatCard`, `SubjectBars`, `WeekChart`.
...
// [100, 91, 82, 74, 67, 61, 55, 50, 45, 41, 37, 33, 30, 27, 25]
...
### Step 2: Implement `MemoryCurve` component

Exact math from `screen-dashboard.jsx`. Converts JSX attribute names to React camelCase.
...
function MemoryCurve() {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
      ...
      {/* SRS fill areas */}
      {segments.map((seg, i) => {
        const segPath = seg.map((p, j) =>
          `${j === 0 ? "M" : "L"} ${x(p[0])} ${y(p[1])}`).join(" ");
...
### Step 3: Implement `StatCard`, `SubjectBars`, `WeekChart`
...
### Step 4: Implement main `DashboardPage` component
...
Expected: no TypeScript errors, no import errors. If TS errors on `AttemptRow` import, verify path `../../lib/supabase/types`.

### Step 6: Smoke test in browser

- [ ] Navigate to `http://localhost:3000/dashboard` — stats hub renders
- [ ] Navigate to `http://localhost:3000/quiz` — quiz session works
- [ ] NavBar logo → `/dashboard` shows stats hub ✓
- [ ] NavBar `/my-stats` still works ✓
- [ ] "복습 N문제 →" links to `/review` ✓
- [ ] "약점 집중" links to `/practice/weakest` ✓
- [ ] "랜덤 세션" links to `/quiz` ✓
- [ ] "로그인" CTA links to `/auth/login` ✓
- [ ] Memory curve SVG renders (teal line + red dashed + markers) ✓
- [ ] Subject bars show with colored dots and 70% threshold line ✓
- [ ] Week chart shows 7 bars ✓

### Step 7: Commit

```bash
git add app/dashboard/page.tsx
git commit -m "feat: replace /dashboard with rich stats hub (memory curve, subject bars, CTA stack, week chart)"
```
...
- [ ] Task 1: `/quiz` route created, quiz session moved
- [ ] Task 2: `.dashboard-2col` CSS class added
- [ ] Task 3: New `/dashboard` stats hub implemented and verified
