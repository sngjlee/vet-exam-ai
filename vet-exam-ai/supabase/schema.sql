...
--   20260713050000  ai_comment_candidates
--
-- Conventions:
...
create index attempts_user_answered_at on public.attempts (user_id, answered_at desc);
...
create table public.comments (
  ...
  question_public_id text                 references public.questions(public_id) on delete cascade,
...
create type public.audit_action as enum (
  ...
  'role_change',
  'ai_comment_publish', 'ai_comment_reject'
);
...
-- Keep comment search compatible with published AI comments, which intentionally
-- reference questions by stable public ID instead of the internal UUID.
...
begin
  ...
  with matches as (
    select
      c.id,
      c.question_public_id as question_id,
      c.type,
      c.body_text,
      c.vote_score,
      c.created_at,
    ...
    join public.questions qs
      on qs.public_id = c.question_public_id
