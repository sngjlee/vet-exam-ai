...
- GitHub Actions `CI`가 `main`과 최근 PR에서 통과하는지 확인합니다. `main` branch protection이 `Lint, typecheck, build` 체크를 머지 전 필수로 강제하는지는 `docs/operations/production-readiness-checklist.md` 8절 기준으로 유지합니다.
